...
\[
  ...
  {\left(\sum_{n = 1}^\infty |a_n|^p\right)^{\fr{1}{p}}}\cdot
  {\left(\sum_{n = 1}^\infty |b_n|^q\right)^{\fr{1}{q}}}\sep
  {\fr{1}{p} + \fr{1}{q} = 1} \sep {p \geq 1}
\]
\[
  {\left(\int_a^b |x(t) y(t)|^p dt \right)^{\fr{1}{p}}} \leq\allowbreak
  {\left(\int_a^b |x(t)|^p dt \right)^{\fr{1}{p}}} \cdot
  {\left(\int_a^b |y(t)|^p dt \right)^{\fr{1}{p}}}
\]

Неравенство Минковского:
\[
  {\left(\sum_{n=1}^\infty|a_n+b_n|^p\right)^{\fr{1}{p}}} \leq\allowbreak
  {\left(\sum_{n=1}^\infty|a_n|^p\right)^{\fr{1}{p}}} +
  {\left(\sum_{n=1}^\infty|b_n|^p\right)^{\fr{1}{p}}}\sep {p\geq 1}
\]
\[
  {\left(\int_a^b|x(t)+y(t)|^p dt \right)^{\fr{1}{p}}} \leq\allowbreak
...
\be
  ...
  \[\rho(x, y) = {\left(\sum_{k = 1}^n |y_k - x_k|^p\right)^{\fr{1}{p}}} \sep
  {p \geq 1}\]
  ...
  Метрика в этом пространстве определяется равенством:
  \[\rho(x, y) = \left(\sum_{n=1}^\infty|y_n-x_n|^p\right)^{\fr{1}{p}} \sep
  {p \geq 1}\]
  ...
  \[ \rho(x,y) = \left(\int_a^b |y(t)-x(t)|^2 dt\right)^{\fr{1}{2}} < \infty \]
  ...
  \[ \int_a^b|x(t)|^p dt \]
  С метрикой:
  \[ \rho(x,y) = \left(\int_a^b|y(t)-x(t)|^pdt\right)^{\fr{1}{p}} \]
...
\[\rho(f,g)= \left( \int_a^b |f(x) - g(x)|^p dx \right) ^{\fr{1}{p}}\]
...
\be
  ...
  \textbf{Например}
  \[f_n(x)=\begin{cases}
    n, & 0 \leq x \leq \fr{1}{n} \\
    0, & \fr{1}{n} \leq x \leq 1
  \end{cases}\]
  \begin{tikzpicture}[>=stealth]
  \begin{axis}[
    restrict y to domain=-1:5,
    ...
    \draw[blue,line width=2pt,] (axis cs:2,0) node[below] {$\fr{1}{n}$} --
...
$R_q^n: (R_p^n)^*=R_q^n \sep p>1 \sep \fr{1}{p}+\fr{1}{q}=1$.
...
\fr{|f(x_0)|}{\Vert x_0 \Vert} = \Vert x_0 \Vert \sep \Vert f \Vert \geq
\Vert x_0 \Vert\]
...
\begin{axis}[
  ...
  \node[below right] at (axis cs:0,0){$t_0$};
\end{axis}
\end{tikzpicture}

Дельта-функция Дирака используется для описания интенсивности
от сосредоточенных воздействий.

Например.

\begin{tikzpicture}[baseline=(current axis.east),>=stealth]
\begin{axis}[
  restrict y to domain=-0.5:2,
  samples=20,
  ticks=none,
  ...
  \draw (axis cs:-3,3) -- (axis cs:3,3) node[right] {$q_0$};
  \draw[->,] (axis cs:-3,3) -- (axis cs:-3,0) node[below] {$x_1$};
  \draw[->,] (axis cs:-1,3) -- (axis cs:-1,0);
  \draw[->,] (axis cs:1,3) -- (axis cs:1,0);
  \draw[->,] (axis cs:3,3) -- (axis cs:3,0) node[below] {$x_2$};
\end{axis}
\end{tikzpicture}
$q(x)=q_0(\theta(x-x_1)-\theta(x-x_2))$.
...
\begin{axis}[
  ...
  \draw[->,] (axis cs:0,3) node[below right] {$Q$}
  -- (axis cs:0,0) node[below] {$x_0$};
\end{axis}
\end{tikzpicture}
$q(x)=Q \delta(x-x_0)$.
...
Рассмотри регулярныю обощенную функцию $T$ порожденнную финкцией $f(t)$.
\[T_\phi = \int_{-\infty}^\infty f(t)\phi(t)dt\]

Возьмем интеграл по частям и воспользуемся финидностью $\phi(t)$.

\[\int_{-\infty}^\infty f'(t)\phi(t)dt
= \underbrace{\phi(t)f(t)\Big|_{-\infty}^\infty}_0
- \int_{\infty}^\infty f(t)\fr{d\phi}{dt}dt
= - T\left(\fr{d\phi}{dt}\right)\]

Здесь порождающая функция $f(t)$ принимается дифференцируемой.
Естественно назвать производную обобщенную $T$ обобщенной функцией,
которая порождается $f'(t)$.
То есть определить для любой регулярной обощенной функции
ее производную равенством
\[\fr{dT}{dt}(\phi)=\int_{-\infty}^\infty f(t)\phi(t)dt
= -T\left(\fr{d\phi}{dt}\right)\]
